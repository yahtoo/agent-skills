...
Invoke the agent-skills:multi-agent-orchestration skill alongside agent-skills:shipping-and-launch.
...
When the runtime supports specialist subagents, spawn each call with an agent type matching the persona's `name` field:
...
- If you need teammates that talk to each other instead of just reporting back, use a runtime that supports interactive agent teams and reference these personas as teammate types (see `references/orchestration-patterns.md`).

**Persona resolution.** This repository ships portable persona prompts in `agents/`. If your Codex runtime exposes custom subagent definitions, prefer the nearest project or user definition for `code-reviewer`, `security-auditor`, or `test-engineer`; otherwise load the matching Markdown persona from this repository.
